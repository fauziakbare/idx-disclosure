...
| 2026-08-21 | v1.8 | Screenshot halaman 1 PDF dikirim ke Telegram bersama pesan summary: `pdf_parser.extract_first_page_image()` render page 1 PNG (PyMuPDF, dpi 150), return key `first_page_image` di `extract_pdf`; `TelegramNotifier.send_photo()` POST `/sendPhoto` (caption batas 1024 char); `main.py` pesan 1 pakai `sendPhoto` saat gambar tersedia, fallback `sendMessage`. Commit `217f5b7`. |
